...
# License: BSD 3-Clause License. For full text see link: https://github.com/DMTF/Redfish-Event-Listener/LICENSE.md
	...
	try:
		...
		if certcheck == 'On':
			if Method == 'GET':
				...
				response = requests.patch(ConfigURI+URILink, data = payload, auth = (LocalUser, LocalPassword), timeout=30)
			elif Method == 'POST':
				response = requests.post(ConfigURI+URILink, data = payload, auth = (LocalUser, LocalPassword), timeout=30)
		else:
			if header == None:
				if Method == 'GET':
					response = requests.get(ConfigURI+URILink, verify=False, auth = (LocalUser, LocalPassword), timeout=30)
				elif Method == 'PATCH':
					header = {"content-type":"application/json"}
					response = requests.patch(ConfigURI+URILink, data = payload, verify=False, auth = (LocalUser, LocalPassword), headers = header, timeout=30)
				elif Method == 'POST':
					header = {"content-type":"application/json"}
					response = requests.post(ConfigURI+URILink, data = payload, verify=False, auth = (LocalUser, LocalPassword), headers = header, timeout=30)
				elif Method == 'CREATE':
					header = {"content-type":"application/json"}
					response = requests.post(ConfigURI+URILink, data = payload, verify=False, headers = header, timeout=30)
				elif Method == 'DELETE':
					header = {"content-type":"application/json"}
					response = requests.delete(ConfigURI+URILink, data = payload, verify=False, auth = (LocalUser, LocalPassword), headers = header, timeout=30)
			else:
				if Method == 'GET':
					response = requests.get(ConfigURI+URILink, verify=False, headers = header, timeout=30)
				elif Method == 'PATCH':
					response = requests.patch(ConfigURI+URILink, data = payload, verify=False, headers = header, timeout=30)
				elif Method == 'POST':
					response = requests.post(ConfigURI+URILink, data = payload, verify=False, headers = header, timeout=30)	
				elif Method == 'DELETE':
					response = requests.delete(ConfigURI+URILink, data = payload, verify=False, headers = header, timeout=30)
...
def PerformSubscription():
	...
	if (len(ServerIPList) == len(UserNameList) == len(PasswordList)) and (len(ServerIPList) > 0) and (not (ServerIPs.strip() == "")):
		...
		for i in range(0, len(ServerIPList)):
			...
			statusCode, Status, body, headers, ExecTime = callResourceURI(ServerIPList[i].strip(), SubscriptionURI, Method = 'POST', payload = payload, header = None, LocalUser = UserNameList[i].strip(), LocalPassword = PasswordList[i].strip())
